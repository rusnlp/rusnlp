...
def make_alpha(files):
    ...
    for num, i in files.items():
        if len(i) > 0:
            ...
            files_alpha[num] = sub('\.+', '.', sub(' \.', '.', sub(' +',' ', sub(r'[^,.a-zA-Z -]+', '', text.replace('-\n', '').replace('\n', ' ')))))
...
if __name__ == '__main__':
    ...
    for a, b, files in walk(path.join('..', '..','parsed', 'texts')):
        for file in files:
            ...
            if get_lang(file.split('.')[0], langs) == 'en':
                with open(path.join(a, file), 'r') as f:
    ...
    for num, eng_file in eng_files.items():
        ...
        files_alpha = make_alpha(eng_files_cleared)
        # make_stemmed(eng_files_cleared)
        make_lemmatized_with_udpipe(files_alpha)
