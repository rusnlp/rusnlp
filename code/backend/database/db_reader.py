...
class ReaderDBase:
    ...
    def select_articles_of_author(self, name):
        ...
        where = "catalogue JOIN author JOIN article JOIN author_alias " \
                "ON author.id=author_alias.author_id AND catalogue.author_id=author.id " \
    ...
    def select_article_by_affiliation(self, affiliation):
        ...
        """
        what = "DISTINCT article.common_id"
        where = "catalogue JOIN author JOIN article ON article.id=article_id " \
    ...
    def select_articles_from_years(self, begin, end):
        what = "DISTINCT article.common_id"
        where = "catalogue JOIN conference JOIN article ON article.id=article_id " \
                "AND conference.id=conference_id"
    ...
    def select_title_by_id(self, article_id):
        ...
        :param article_id: string
            ...
            (in format %s_%s_%s:(conference, year, lowercased underlined paper's title))
        :return: string
    ...
    def select_author_by_id(self, article_id):
        ...
        """
        what = '''DISTINCT alias'''
        where = '''author JOIN catalogue JOIN article JOIN author_alias 
        ON author.id=catalogue.author_id AND article_id=article.id 
        AND author.id=author_alias.author_id'''
        condition = '''common_id="{}"'''.format(str(article_id))
    ...
    def select_author_simple_by_id(self, article_id):
        what = '''DISTINCT name'''
        where = '''author JOIN catalogue JOIN article ON author.id=catalogue.author_id 
    ...
    def select_by_id(self, what, article_id):
        ...
        where = '''article INNER JOIN catalogue INNER JOIN conference ON article.id=article_id 
        AND conference_id=conference.id'''
        condition = '''common_id="{}"'''.format(str(article_id))
        return self._bd.select(what, where, condition)[0][0]
    ...
    def select_affiliation_by_id(self, article_id):
        where = "catalogue JOIN author JOIN article ON article.id=catalogue.article_id " \
                "AND author.id=catalogue.author_id"
        affiliations = list(set([i[0] for i in self._bd.select(
            "author.affiliation", where, '''common_id="{}"'''.format(article_id))]))
        aff_list = []
    ...
    def select_aff_clusters_by_id(self, common_id):
        where = "catalogue JOIN author JOIN article JOIN affiliation_alias " \
                "ON article.id=catalogue.article_id AND author.id=catalogue.author_id " \
                "AND affiliation_alias.author_id=author.id"
        cluster = set([i[0] for i in
                       self._bd.select("cluster", where, '''common_id="{}"'''.format(common_id))])
        return cluster
    ...
    def select_affiliation_by_language(self, lang):
        ...
        return set([re.sub("^, ", '', i[0].replace("\n", "")).strip() for i in
                    self._bd.select("affiliation", where, condition)])
    ...
    def select_cluster_author_by_common_id(self, common_id):
        where = "catalogue JOIN author JOIN article JOIN author_alias " \
                "ON article.id=catalogue.article_id AND author.id=catalogue.author_id " \
                "AND author_alias.author_id=author.id"
    ...
    def select_all_from_column(self, column, condition=None):
        ...
        """
        where = '''catalogue JOIN conference JOIN article JOIN author JOIN author_alias 
        JOIN affiliation_alias ON author.id=author_alias.author_id 
        AND catalogue.conference_id=conference.id AND catalogue.article_id=article.id 
        AND author.id=catalogue.author_id AND affiliation_alias.author_id=author.id'''
    ...
    def select_affiliation_by_cluster(self, cluster_id):
        all_names = [i[0] for i in
                     self._bd.select("alias", "affiliation_alias", "cluster={}".format(cluster_id))]
    ...
    def update_statistics(self):
        ...
        # df.loc[5]=('Amount of Russian authors',
        # len(self.select_all_from_column("author_alias.alias", '''language="ru"''')))
        df.loc[5] = ('Amount of English authors',
                     len(self.select_all_from_column("author_alias.alias", '''language="en"''')))
        # df.loc[7]=('Amount of Russian affiliations',
        # len(self.select_all_from_column("affiliation_alias.cluster", '''language="ru"''')))
        df.loc[6] = ('Amount of Eglish affiliations', len(
            self.select_all_from_column("affiliation_alias.cluster", '''language="en"''')))
    ...
    def get_dict_of_conference_description(self, confname):
        self._bd.cursor.execute(
            ...
            'WHERE conference.conference="{}"'.format(confname))
